/**
 * Official SentriX AI contact channels.
 * Edit ONLY this file to update links / handles.
 * Leave url as "" until the official link is verified — do not invent data.
 */
export const brand = {
  name: "SentriX AI",
  tagline: {
    ar: "AI-Powered Workplace Safety",
    en: "AI-Powered Workplace Safety",
  },
};

export const contactChannels = [
  {
    id: "whatsapp",
    platform: { ar: "واتساب", en: "WHATSAPP" },
    handle: {
      ar: "+968 90985654",
      en: "+968 90985654",
    },
    url: "https://wa.me/96890985654",
  },
  {
    id: "instagram",
    platform: { ar: "إنستغرام", en: "INSTAGRAM" },
    handle: {
      ar: "@sentrix_ai",
      en: "@sentrix_ai",
    },
    url: "https://www.instagram.com/sentrix_ai?igsi=NmE2enc5NnRoN2o1",
  },
  {
    id: "linkedin",
    platform: { ar: "لينكدإن", en: "LINKEDIN" },
    handle: {
      ar: "تابع SentriX AI على LinkedIn",
      en: "Follow SentriX AI on LinkedIn",
    },
    url: "https://www.linkedin.com/company/sentrixai-oman/",
  },
  {
    id: "phone",
    platform: { ar: "الهاتف", en: "PHONE" },
    handle: {
      ar: "+968 90985654",
      en: "+968 90985654",
    },
    url: "tel:+96890985654",
  },
  {
    id: "email",
    platform: { ar: "البريد", en: "EMAIL" },
    // Waiting for official email — do not invent an address
    handle: {
      ar: "ADD_OFFICIAL_EMAIL",
      en: "ADD_OFFICIAL_EMAIL",
    },
    url: "",
  },
];
